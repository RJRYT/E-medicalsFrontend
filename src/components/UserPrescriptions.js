...
function UserPrescriptions() {
    ...
    const [array,setArray]= useState([{doctorid:{},date:''}]);
    const id=localStorage.getItem('userid')

    useEffect(()=>{
        axiosInstance.post(`/viewPrescriptionByUserId/${id}`)
        .then((response)=>{
                            ...
                            
                                {
                                    ...
                                    <div class="col-sm-12 col-md-8 d-flex align-items-center">
                                        <div class="text-start ps-4">
                                            <h5 class="mb-3">Dr. {a.doctorid.name}</h5>
                                    ...
                                    <div class="col-sm-12 col-md-4 d-flex flex-column align-items-start align-items-md-end justify-content-center">
                                        <div class="d-flex mb-3">
                                            <Link class="btn" id='btns_bg' to={`/user_view_prescription/${a.appointmentid}`}>View Details</Link>
                                        </div>
                                    </div>
                                </div>
                                </div>
                                        )
                                    }) : <h1 style={{padding:'30px',textAlign:'center'}}>No Prescriptions</h1>
                                }
